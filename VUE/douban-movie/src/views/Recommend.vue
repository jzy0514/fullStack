<template>
  <div class="recommend">
    <!-- 首页整个页面多支持better-scroll滚动 -->
    <ScrollView>
      <ListBlock :movies="playingMovies" :title="`正在热映(${playingCount})`" @more="goMore(0)" @select="selectItem"></ListBlock>
      <div class="space" style="background-color: #f6f6f6; height: 10px"></div>
      <ListBlock :movies="commingMovies" :title="`即将上映(${commingCount})`" @more="goMore(1)" @select="selectItem"></ListBlock>
    </ScrollView>
  </div>
</template>

<script>
import axios from 'axios'
import ListBlock from '@/components/global/ListBlock'
export default {
  data () {
    return {
      playingMovies: [],
      playingCount: 0,
      commingMovies: [],
      commingCount: 0
    }
  },
  components: {
    ListBlock
  },
  created() {
    // console.log('wdnmd')
    this.getMovie()
  },
  methods: {
    getMovie() {
        axios.get('https://www.easy-mock.com/mock/5ca495f2ea0dc52bf3b67fd7/jzy/bouban-movies').then(res => {
        console.log(res,'.....')
        if (res.data.code === 1001) {
          const { comming, playing } = res.data.result
          this.playingMovies = playing.movies
          this.playingCount = playing.count
          // console.log(comming)
          this.commingMovies = comming.movies
          this.commingCount = comming.count  
        }
      })
    },
    selectItem(id) {
      console.log(id)
      this.$router.push(`/movie/${id}`)
    },
    goMore(type) {
      console.log(type)
    }
  }
}
</script>

<style lang="stylus" scoped>
.recommend
  height 100%
</style>
